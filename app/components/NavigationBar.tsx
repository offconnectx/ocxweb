    ...
    >
      <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto px-6">
        <motion.a
          ...
          animate="visible"
        >
          <img src="/off.png" className="h-10" alt="Logo" /> {/* Increased logo size */}
        ...
        >
          <ul className="font-medium flex flex-col p-4 md:p-0 mt-4 rounded-lg md:flex-row md:space-x-8 rtl:space-x-reverse md:mt-0 md:border-0 bg-transparent">
            {["Home", "Features", "Contacts", "Whitepaper"].map((item, i) => (
              <motion.li
                key={item}
                custom={i}
              ...
              >
                <ScrollLink
                  to={item.toLowerCase() === "whitepaper" ? "about" : item.toLowerCase()}
                  smooth={true}
                  duration={500}
...
const ToggleMenuButton = ({ toggleMenu, isMenuOpen }: { toggleMenu: VoidFunction, isMenuOpen: boolean }) => {
  return (
    <motion.button
      ...
      onClick={toggleMenu}
      type="button"
      className="inline-flex items-center p-2 w-12 h-12 justify-center text-sm rounded-lg md:hidden hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200 text-gray-800 dark:text-white dark:hover:bg-[#275a7d] dark:focus:ring-[#275a7d]"
